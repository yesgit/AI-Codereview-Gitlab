...
# 硬编码的默认提示词（作为文件加载失败时的回退）
DEFAULT_SYSTEM_PROMPT = r"""你是一位资深的软件开发工程师，专注于代码的规范性、功能性、安全性和稳定性。本次任务是对员工的代码进行审查，具体要求如下：
...
DEFAULT_USER_PROMPT = """以下是某位员工向 GitLab 代码库提交的代码，请以{{ style }}风格审查以下代码。
...
async def get_default_prompts():
    """获取默认的提示词模板"""
    try:
        # 获取 conf/prompt_templates.yml 的路径
        ...
        code_review = templates.get('code_review_prompt', {})
        return {
            'custom_prompt_system': code_review.get('system_prompt', ''),
            'custom_prompt_user': code_review.get('user_prompt', '')
        }
