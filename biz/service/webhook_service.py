...
class WebhookService:
    ...
    def create_or_update_webhook_mapping(project_name: Optional[str] = None, url_slug: Optional[str] = None,
        ...
        try:
            ...
            ins = text('''INSERT INTO project_webhooks (project_name, url_slug, dingtalk_url, feishu_url, wecom_url, custom_prompt_system, custom_prompt_user, created_at, updated_at)
                         VALUES (:project_name, :url_slug, :dingtalk_url, :feishu_url, :wecom_url, :custom_prompt_system, :custom_prompt_user, :created_at, :updated_at)''')
            upd = text('''UPDATE project_webhooks SET project_name = :project_name, url_slug = :url_slug, dingtalk_url = :dingtalk_url, feishu_url = :feishu_url, wecom_url = :wecom_url, custom_prompt_system = :custom_prompt_system, custom_prompt_user = :custom_prompt_user, updated_at = :updated_at WHERE id = :id''')
            with engine.begin() as conn:
                ...
                if existing_project and existing_slug:
                    ...
                    conn.execute(upd, {
                        'project_name': project_name, 'url_slug': url_slug, 'dingtalk_url': dingtalk_url,
    ...
    def get_all_webhook_mappings():
        try:
            engine = get_engine()
            sql = text('SELECT id, project_name, url_slug, dingtalk_url, feishu_url, wecom_url, custom_prompt_system, custom_prompt_user, created_at, updated_at FROM project_webhooks')
            with engine.connect() as conn:
