...
def handle_queue(function: callable, data: any, token: str, url: str, url_slug: str,
    ...
    if queue_driver == 'rq':
        try:
            ...
            # 将任务加入 Redis 队列
            job = q.enqueue(function, data, token, url, url_slug, comment_url, comment_token, review_strategy, comment_project_path, comment_project_id, job_timeout='30m')
            ...
            # 失败时回退到多进程模式
            _handle_with_multiprocessing(function, data, token, url, url_slug, comment_url, comment_token, review_strategy, comment_project_path, comment_project_id)
    else:
        # 使用多进程模式（默认）
        _handle_with_multiprocessing(function, data, token, url, url_slug, comment_url, comment_token, review_strategy, comment_project_path, comment_project_id)
...
def _handle_with_multiprocessing(function: callable, data: any, token: str, url: str, url_slug: str,
                                 ...
                                 comment_project_id: int = None):
    """使用多进程处理任务"""
    process = Process(target=function, args=(data, token, url, url_slug, comment_url, comment_token, review_strategy, comment_project_path, comment_project_id))
    process.start()
...
def retry_task(function: callable, data: any, token: str, url: str, url_slug: str,
    ...
    :param comment_url: 评论目标 URL（可选）
    :param comment_token: 评论目标 Token（可选）
    :param comment_project_id: 评论目标数字项目 ID（可选）
    :param delay: 延迟秒数
    """
    queue_driver = os.getenv('QUEUE_DRIVER', 'multiprocessing').lower()
    ...
    if queue_driver == 'rq':
        try:
            ...
            # 失败时使用多进程模式重试
            _retry_with_multiprocessing(function, data, token, url, url_slug, comment_url, comment_token, review_strategy, comment_project_path, comment_project_id, delay)
    else:
        # 使用多进程模式重试
        _retry_with_multiprocessing(function, data, token, url, url_slug, comment_url, comment_token, review_strategy, comment_project_path, comment_project_id, delay)
...
def _retry_with_multiprocessing(function: callable, data: any, token: str, url: str, url_slug: str,
                                ...
                                review_strategy: str = 'diff_only',
                                comment_project_path: str = None,
                                comment_project_id: int = None, delay: int = 60):
    """使用多进程延迟重试任务"""
    def delayed_retry():
        time.sleep(delay)
        process = Process(target=function, args=(data, token, url, url_slug, comment_url, comment_token, review_strategy, comment_project_path, comment_project_id))
